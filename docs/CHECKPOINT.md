...
- **SP26 Backend deploy to Render — DONE & LIVE** (PR #18 merged to main,
  `fe2de9d`; branch deleted). **Live URL: `https://halal-scanner-api-xotr.onrender.com`**
  (free tier, Oregon). NOT the clean `halal-scanner-api.onrender.com` — that
  subdomain was already taken globally by an unrelated "Halal Food Scanner API"
  v1.2.0, so our service got the `-xotr` suffix; always use the suffixed URL.
  committed `render.yaml` Blueprint: Docker web service (installs `tesseract-ocr`
  in image for `/scan-image` in prod) + managed PostgreSQL database. New `postgres`
  extra in `pyproject.toml` (`psycopg[binary]>=3.1`); `_normalize_db_url` in `db.py`
  adapts the Render Postgres URL scheme to psycopg3 (`postgresql://` →
  `postgresql+psycopg://`). Blueprint sets `HALAL_ENV=production` (docs hidden,
  rate limiting enforced), `HALAL_RATE_LIMIT=60`, `HALAL_JWT_SECRET` auto-generated,
  wires `HALAL_DATABASE_URL` from managed Postgres; two `sync: false` env vars
  (`HALAL_ADMIN_EMAILS`=operator email, `HALAL_CORS_ORIGINS`=blank) filled on the
  dashboard. `mobile/.env` `EXPO_PUBLIC_API_URL` now points at the prod URL.
  Verified end-to-end against prod: `/health` 200, `/docs`+`/openapi.json` 404,
  register→login→API key→`/classify` all 200 (Postgres + auth + rulebook work).
  **Render gotcha:** `render.yaml` declares no `branch:`, so the web service tracks
  whatever branch the Blueprint reads from — repointed to `main` post-merge in the
  dashboard, so future merges to main auto-redeploy. Free tier spins down after
  ~15 min idle (~30–60s cold start on next request).
...
Backend is **deployed and live on Render** (`https://halal-scanner-api-xotr.onrender.com`),
verified end-to-end; `mobile/.env` points at it. Candidate next steps for the app/backend:
